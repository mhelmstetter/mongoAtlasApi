...
def __post_alert_configs(args,alert_configs):
    ...
    for alert in alert_configs['results']:
        ...
        if args.continueOnError and (response.status_code != requests.codes.created):
            ...
            #print( "Failed to create alert id: %s" % (alert['id']))
            #print(json.dumps(alert))
            response.raise_for_status()
            migrated_alerts += 1
...
alertConfigsEndpoint = "https://cloud.mongodb.com/api/atlas/v1.0/groups/" + args.group +"/alertConfigs"
